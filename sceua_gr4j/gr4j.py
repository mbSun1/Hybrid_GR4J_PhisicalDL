...
class GR4J(object):
    ...
    def __init__(self, s_init=0.5, r_init=0.6):
        self.s_init = s_init
        self.r_init = r_init
    ...
    def run(self, prec, etp):
        ...
        for t in range(1, num_timesteps + 1):
            ...
            # update state of rain, distributed through the unit hydrograph
            ...
            # update routing store更新汇流存储
            r_store[t] = max(0, r_store[t - 1] + uh1[0] + gw_exchange)

            # outflow of routing store汇流存储流出
        ...
        qsim = qsim[1:].detach().reshape(-1, 1).squeeze_(1).tolist()#(100,1)
        s_store = s_store[1:].detach().reshape(-1, 1).tolist()
        r_store = r_store[1:].detach().reshape(-1, 1).tolist()
